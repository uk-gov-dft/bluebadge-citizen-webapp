...
public class ActuatorPage {
  ...
  @Autowired
  public ActuatorPage(CommonPage commonPage) {
  ...
  public void openActuator(String actuatorUrl) {
    ...
    try {
      commonPage.getWebDriver().get(actuatorBaseUrl + actuatorUrl);
      commonPage.getWebDriver().manage().window().maximize();
      checkForError();
    } catch (Exception ex) {
  ...
  private JsonNode parseResponse(boolean failOnError) {
    WebElement pre = commonPage.findElementWithCSSSelector("pre");
