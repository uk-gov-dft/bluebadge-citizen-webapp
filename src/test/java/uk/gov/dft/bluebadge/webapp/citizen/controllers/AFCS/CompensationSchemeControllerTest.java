...
public class CompensationSchemeControllerTest {
    ...
    public void show_ShouldDisplayCompensationScheme_WithRadioOptions() throws Exception {
        RadioOptionsGroup options = new RadioOptionsGroup("afcs.compensationSchemePage.title")
                .autoPopulateBooleanOptions();

        CompensationSchemeForm form = CompensationSchemeForm.builder().build();

        mockMvc.perform(get("/lump-sum"))
    ...
    public void submit_ShouldDisplayErrors_WhenNoOptionsAreSelected() throws  Exception {
        ...
        mockMvc.perform(post("/lump-sum").param("hasReceivedCompensation", ""))
                .andExpect(status().isFound())
                //.andExpect(model().attributeHasFieldErrorCode("formRequest", "hasReceivedCompensation", "NotNull"))
