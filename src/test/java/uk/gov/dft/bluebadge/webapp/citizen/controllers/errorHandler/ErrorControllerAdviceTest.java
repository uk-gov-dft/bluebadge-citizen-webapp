...
public class ErrorControllerAdviceTest {
    ...
    @Test
    public void handleHttpException_shouldReturnRedirectToErrorTemplateAndPopulateRedirectAttributes() throws JsonProcessingException {
        CommonResponse commonResponse = new CommonResponse();
        commonResponse.error(
                new Error().message("some message")
                        .code(500)
                        .reason("no reason")
                        .errors(Lists.newArrayList(new ErrorErrors()))
        ).id("someId").context("context");
        ...
        when(writerMock.writeValueAsString(ex.getCommonResponse())).thenReturn("some api client error message");

        String template = controllerAdvice.handleClientApiException(ex, reqMock, redirectAttributesMock);
        assertThat(template).isEqualTo("redirect:/something-went-wrong");
        verify(redirectAttributesMock).addFlashAttribute("commonResponse", "some api client error message");
        verify(redirectAttributesMock, times(1)).addFlashAttribute("exception", ex);
    }
}
