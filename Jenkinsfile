                  ...
                  withCredentials([string(credentialsId: 'GITHUB_TOKEN', variable: 'GITHUB_TOKEN')]) {
                    try {
                        sh '''
                            ...
                            docker run -d --network "dev-env-develop_default" --rm --name zalenium -p 4444:4444 -v /var/run/docker.sock:/var/run/docker.sock dosel/zalenium start --desiredContainers 8 --maxTestSessions 2 --keepOnlyFailedTests true --videoRecordingEnabled false --maxDockerSeleniumContainers 8
